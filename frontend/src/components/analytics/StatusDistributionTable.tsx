import type { StatusDistributionItemDto } from '../../api/types'
import { formatStoryPoints } from '../../utils/format'
import { StatusCategoryBadge } from '../common/StatusCategoryBadge'

interface StatusDistributionTableProps {
  items: StatusDistributionItemDto[]
}

export function StatusDistributionTable({ items }: StatusDistributionTableProps) {
  if (items.length === 0) {
    return (
      <p className="text-sm text-gray-500">No status distribution data for this sprint.</p>
    )
  }

  return (
    <div className="overflow-x-auto rounded-lg border border-gray-200">
      <table className="min-w-full divide-y divide-gray-200 text-sm">
        <thead className="bg-gray-50">
          <tr>
            <th className="px-4 py-3 text-left font-medium text-gray-600">Status</th>
            <th className="px-4 py-3 text-left font-medium text-gray-600">Category</th>
            <th className="px-4 py-3 text-right font-medium text-gray-600">Count</th>
            <th className="px-4 py-3 text-right font-medium text-gray-600">Story Points</th>
          </tr>
        </thead>
        <tbody className="divide-y divide-gray-100 bg-white">
          {items.map((item) => (
            <tr key={`${item.status}-${item.statusCategory}`}>
              <td className="px-4 py-3 font-medium text-gray-900">{item.status}</td>
              <td className="px-4 py-3">
                <StatusCategoryBadge category={item.statusCategory} />
              </td>
              <td className="px-4 py-3 text-right text-gray-700">{item.count}</td>
              <td className="px-4 py-3 text-right text-gray-700">
                {formatStoryPoints(item.storyPoints)}
              </td>
            </tr>
          ))}
        </tbody>
      </table>
    </div>
  )
}
